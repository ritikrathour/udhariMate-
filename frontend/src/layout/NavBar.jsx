                            ...
                            <div className="relative flex justify-center items-center gap-2" >
                                ...
                                <div className={`${show ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0 pointer-events-none"}  absolute bg-white border w-[220px] z-40  flex flex-col items-start top-16 shadow rounded-md right-5 transition-all`}>
                                    ...
                                    <ul className="flex flex-col justify-start items-start gap-1 p-1 w-full mt-1">
                                        <Link to="/" className="hover:text-gray-800 hover:bg-slate-100 hover:backdrop-blur-lg p-1 rounded-md w-full text-gray-500 align-middle transition-all text-sm font-medium"><i className="fas fa-home mr-4 w-[10px] h-[10px]"></i>Home</Link>
                                        {
                                            user?.role === "shopkeeper" ? (
                                                <div className="flex flex-col gap-2 w-full">
                                                    <Link to="/ladger" className="hover:text-gray-800 hover:bg-slate-100 hover:backdrop-blur-lg p-1 rounded-md w-full text-gray-500 transition-all text-sm font-medium"><i className="fas fa-users mr-4 w-[10px] h-[10px]"></i>Ladgers</Link>
                                                </div>

                                            ) : (
                                                <div className="flex flex-col gap-2 w-full">
                                                    <Link to={`/transacation-area/${user?.shopId}`} className="hover:text-gray-800 hover:bg-slate-100 hover:backdrop-blur-lg p-1 rounded-md w-full text-gray-500 transition-all text-sm font-medium">
                                        ...
                                        <Link to={`/profile/${user?._id}`} className="hover:text-gray-800 hover:bg-slate-100 hover:backdrop-blur-lg p-1 rounded-md w-full text-gray-500 align-middle transition-all text-sm font-medium"><i className="fas fa-eye mr-4 w-[10px] h-[10px]"></i>Profile</Link>

                                        <button onClick={() => showSignOutPopUp()} type="button" className="text-sm text-left hover:bg-slate-100 hover:backdrop-blur-lg p-1 rounded-md w-full text-gray-500 font-medium hover:text-gray-800 transition-all"><i className="fas fa-sign-out mr-4 w-[10px] h-[10px]"></i>Sign Out</button>
                                    </ul>
                                </div>
                            </div>
                        ) : (
